...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      // Create session entry if it exists
      if (data.sessionId && !conversations.has(data.sessionId)) {
      ...
      const conversation = data.sessionId ? conversations.get(data.sessionId) : null;
