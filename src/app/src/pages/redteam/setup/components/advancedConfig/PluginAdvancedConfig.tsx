...
import { DEFAULT_NUM_TESTS, MIN_TESTS, MAX_TESTS, availableStrategies } from './constants';

interface PluginAdvancedConfigProps {
  plugin: string;
  isExpanded: boolean;
  config: PluginConfig;
  onUpdateConfig: (plugin: string, newConfig: PluginConfig) => void;
}
          ...
          <Stack spacing={2}>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {availableStrategies.map((strategy: { id: string; name: string }) => (
                  <Chip
                    key={strategy.id}
                    label={strategy.name}
                    size="small"
